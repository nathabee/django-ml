...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
              ...
              await Promise.all(
                reportCatalogue.resultats.map(async (resultat: Resultat) => {
                  if (resultat.groupage.groupage_icon_id) {
                    //const imageKey = pomolobee_icon_${resultat.groupage.groupage_icon_id};
                    const imageKey = `pomolobee_icon_${resultat.groupage.groupage_icon_id}`;
  ...
  const logout = () => {
    if (typeof window !== 'undefined') {
      ...
      // Remove all items from local storage that start with 'pomolobee_'
      Object.keys(localStorage).forEach(key => {
        if (key.startsWith('pomolobee_')) {
